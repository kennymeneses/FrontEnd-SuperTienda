...
const [error, setError] = useState("");
    ...
    const authentication = e =>{
        ...
        fetch(url,miinit).then(r=>{console.log(r); return r.json()}).then(r=>{if(r.apiEstado == "error"){setError(r.apiMensaje);aviso_error.current.style.display="flex"}
                else{console.log(r);aviso_error.current.style.display="none"; localStorage.setItem('token', r.token);localStorage.setItem('idUsuario',r.id);setError('');console.log('Se inicio sesion!')}})
    }
